...
```bash
cd extension
zip -r ../ksef-monitor-1.0.1.zip . --exclude="*.DS_Store"
```
...
- `refreshToken` → **7 dni** (pole `refreshToken.validUntil` w odpowiedzi API)
...
pollState: {
  ...
  needsPin:          boolean,      // crypto-lock: refresh token wygasł (7 dni)
...
```js
// Session storage (RAM, czyszczony przy zamknięciu przeglądarki)
chrome.storage.session: { ksefTokenPlain: string }  // odszyfrowany token KSeF
```
...
- **Hierarchia auth w background.js** (getOrRefreshAccessToken):
    1. accessToken z cache (ważny ~15 min)
    2. refreshToken z dysku → silent refresh (ważny 7 dni)
    3. Token KSeF z `chrome.storage.session` → pełna re-auth (dostępny dopóki przeglądarka otwarta)
    4. PIN wymagany → `needsPin=true`, polling staje
- **needsPin=true** → crypto-lock (refresh token wygasł po 7 dniach lub session pusta po restarcie), badge `!`
- **PIN_TIMEOUT_MS (4h)** → UI-lock tylko w popup, background polluje normalnie. NIE weryfikuje PIN kryptograficznie – świadoma decyzja.
...
- **Walidacja tokenu** → strict regex na pełny format (reverse-engineering z próbki produkcyjnej, brak oficjalnej spec formatu)
- **refreshTokenExpiry** → czytamy `data.refreshToken.validUntil` (nie `data.refreshTokenExpiry` – tego pola nie ma)
...
- Status: **opublikowane** (v1.0.0 od 16.03.2026, v1.0.1 w recenzji)
...
- Sticky nagłówki sekcji w liście faktur (przy dużej liczbie faktur 20+) – backlog, uruchomić jeśli pojawi się feedback
- Date range picker dla progu „oczekujących" (cross-platform, bez native `<input type="date">`)

### 🟡 Techniczny

- Weryfikacja regex tokenu na kolejnych realnych tokenach (format zweryfikowany na 1 próbce – JDG/os. fizyczna; nieznany format dla spółek/pieczęć)
...
2. Powiedz: _„Kontynuujemy KSeF Monitor v1.0.1"_
