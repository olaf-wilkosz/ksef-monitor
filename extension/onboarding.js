...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('inputToken').addEventListener('input', function () {
    ...
    const TOKEN_RE = /^(\d{4})(\d{2})(\d{2})-[A-Z0-9]{2}-[A-F0-9]{10}-[A-F0-9]{10}-[A-Z0-9]{2}\|nip-(\d{10})\|[a-f0-9]+$/;
  ...
  document.getElementById('btnRetryTest').addEventListener('click', () => {
    try { chrome.runtime.sendMessage({ type: 'CLEAR_BACKOFF' }); } catch {}
    resetTestUI();
    runTokenTest();
  });
...
async function lookupCompanyName(nip) {
  ...
  try {
    ...
    if (!res.ok) { setCompanyBadge(null); return; }
...
async function runTokenTest() {
  ...
  try {
    ...
    if (!probe.ok) {
      setIcon('iconKey', '❌');
      showError(
        `Serwer KSeF zwrócił HTTP ${probe.status}`,
    ...
    showError(
      ...
      'Sprawdź połączenie internetowe lub status serwisów MF.',
      ['Sprawdź połączenie internetowe', 'Status serwisów: ksef.podatki.gov.pl']
    );
    return;
  }
  setIcon('iconKey', '✅');
    ...
    if (response?.ok) {
      ...
      if (is450) {
        showError(
          ...
          ['Token został unieważniony w portalu KSeF',
           'Tokeny wygasają 31.12.2026 – sprawdź datę ważności'],
          'back'
        );
      } else {
        showError(
...
async function confirmPin() {
  ...
  if (pin.length < 4) { errEl.textContent = 'Wpisz 4-cyfrowy PIN'; return; }
  if (!/^\d{4}$/.test(pin)) { errEl.textContent = 'PIN musi składać się z cyfr'; return; }
  ...
  try {
    ...
    if (response?.ok) {
      ...
      setPanel(
        ...
        count > 0
          ? `${count} nieprzejrzanych z bieżącego miesiąca.`
          : 'Brak faktur z bieżącego miesiąca.'
      );
      await chrome.runtime.sendMessage({
        type: 'UPDATE_INTERVAL', minutes: pendingConfig.pollIntervalMinutes,
      });
    } else {
      setPanel(
        'fetchPanel', 'error',
